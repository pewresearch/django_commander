...
from django_loaders.models import LoaderLog, Loader
from django_loaders.utils import MissingDependencyException


def log_loader(handle):
    def wrapper(self, *args, **options):
        self.loader = Loader.objects.create_or_update({
            "name": self.name,
            "parameters": str(self.parameters)
        })
        self.log = LoaderLog.objects.create(
            loader=self.loader,
        ...
        try:
            ...
            if self.log:
                ...
                try: self.log.save()
                    ...
                    self.log = LoaderLog.objects.get(pk=self.log_id)
                    self.log.end_time = datetime.datetime.now()
                    self.log.save()
...
def cache_results(func):
    ...
    def wrapper(self, *args, **options):
        ...
        if not is_not_null(data) or self.options["refresh_data"] or options.get("refresh_data", False):
            print "Refreshing data from source for downloader '%s.%s'" % (str(self.__class__.name), str(func.__name__))
            data = func(self, *args)
...
class BasicLoader(object):
    ...
    """
    All Basic loaders require an init function that have the following
         self.name
    ...
    def __init__(self, *args, **options):
        ...
        self.cache = CacheHandler("loaders/{}".format(self.name))
    ...
    def check_dependencies(self):
        ...
        if hasattr(self, "dependencies"):
            ...
            for d, params in self.dependencies:
                logs = LoaderLog.objects.filter(loader__name=d).filter(end_time__isnull=False).filter(error__isnull=True)
                for p in params:
                    ...
                    logs = logs.filter(loader__parameters__regex=r"[\"']?%s[\"']?\: [\"']?%s[\"']?" % (p, params[p]))
                if logs.count() == 0:
...
class DownloadIterateLoader(BasicLoader):
...
class IterateDownloadLoader(BasicLoader):
...
class MultiprocessedIterateDownloadLoader(BasicLoader):
...
class MultiprocessedDownloadIterateLoader(BasicLoader):
...
def loader_multiprocess_wrapper(loader_name, parameters, options, *args):

    params = {}
    params.update(parameters)
    params.update(options)
    reset_django_connection()
    from logos.loaders import loaders
    return loaders[loader_name](**params).parse_and_save(*args)
